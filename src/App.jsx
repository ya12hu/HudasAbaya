...
export default function App() {
  ...
  const exportOrders=()=>{
    ...
    const csv=[headers,...rows].map(r=>r.map(v=>{const s=String(v||"").replace(/"/g,'""');return '"'+s+'"';}).join(",")).join("\n");
    ...
    <div dir={isAr?"rtl":"ltr"} style={{minHeight:"100vh",background:"#faf7f2",color:"#1a1208",fontFamily:"Georgia,serif",overflowX:"hidden",width:"100%",maxWidth:"100vw",position:"relative"}}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .hero-btn{background:linear-gradient(135deg,#c4922a,#e8c060,#c4922a);background-size:200% auto;color:#1a0d03;border:none;padding:16px 52px;border-radius:40px;cursor:pointer;font-family:"DM Sans",sans-serif;font-size:.88rem;font-weight:700;letter-spacing:.18em;text-transform:uppercase;transition:all .4s;box-shadow:0 4px 20px rgba(196,146,42,.35)}
        .hero-btn:hover{background-position:right center;transform:translateY(-3px);box-shadow:0 8px 35px rgba(196,146,42,.55);letter-spacing:.24em}
        html,body{overflow-x:hidden;width:100%;max-width:100vw}
        *{-webkit-tap-highlight-color:transparent}
        /* Responsive breakpoints */
        @media(min-width:1024px){
          .mobile-grid{grid-template-columns:repeat(4,1fr)!important}
          .mobile-cats{grid-template-columns:repeat(6,1fr)!important}
          .admin-grid{grid-template-columns:repeat(4,1fr)!important}
          .detail-grid{grid-template-columns:1fr 1fr!important}
        }
        @media(min-width:768px) and (max-width:1023px){
          .mobile-grid{grid-template-columns:repeat(3,1fr)!important}
          .mobile-cats{grid-template-columns:repeat(4,1fr)!important}
      ...
      <header style={{background:"rgba(255,252,248,.96)",backdropFilter:"blur(12px)",borderBottom:"1px solid #ede5d8",position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 20px rgba(0,0,0,.04)"}}>
        <div style={{maxWidth:1300,margin:"0 auto",padding:"0",width:"100%"}}>
          {/* Row 1: Nav + Social + Actions */}
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 12px",height:36,borderBottom:"1px solid #f0e8da",background:"#fff"}}>
            {/* Nav links */}
            <div style={{display:"flex",alignItems:"center",gap:2}}>
              <button className="nav-link" onClick={()=>setView("home")} style={{fontFamily:F,fontSize:".68rem",color:view==="home"?"#8B6914":"#999",borderBottom:view==="home"?"2px solid #8B6914":"2px solid transparent",padding:"6px 8px",whiteSpace:"nowrap"}}>{t.store}</button>
              <button className="nav-link" onClick={()=>setView("wishlist")} style={{fontFamily:F,fontSize:".68rem",color:view==="wishlist"?"#8B6914":"#999",borderBottom:view==="wishlist"?"2px solid #8B6914":"2px solid transparent",padding:"6px 8px",whiteSpace:"nowrap",position:"relative"}}>
                {t.wishlistNav}
                {wishlist.length>0&&<span style={{position:"absolute",top:2,right:-2,background:"#c0392b",color:"#fff",borderRadius:"50%",width:13,height:13,display:"flex",alignItems:"center",justifyContent:"center",fontSize:".5rem",fontFamily:"DM Sans,sans-serif",fontWeight:700}}>{wishlist.length}</span>}
              </button>
              <button className="nav-link" onClick={()=>{setView("track");setTrackedOrder(null);setTrackSearched(false);}} style={{fontFamily:F,fontSize:".68rem",color:view==="track"?"#8B6914":"#999",borderBottom:view==="track"?"2px solid #8B6914":"2px solid transparent",padding:"6px 8px",whiteSpace:"nowrap"}}>{t.trackNav}</button>
            </div>
            {/* Social + Lang + Cart */}
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <a href="https://snapchat.com/t/KurOUTVy" target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",justifyContent:"center",background:"#FFFC00",borderRadius:"50%",width:22,height:22,textDecoration:"none",fontSize:".75rem",flexShrink:0}}>👻</a>
              <a href="https://www.instagram.com/hudas_abaya_boutique" target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)",borderRadius:"50%",width:22,height:22,textDecoration:"none",flexShrink:0}}>
              ...
              <a href="https://www.tiktok.com/@hudas.abaya" target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",justifyContent:"center",background:"#000",borderRadius:"50%",width:22,height:22,textDecoration:"none",flexShrink:0}}>
              ...
              <button onClick={()=>setLang(l=>l==="en"?"ar":"en")} style={{background:"#f0e4cc",border:"none",color:"#7a5012",padding:"3px 8px",borderRadius:20,cursor:"pointer",fontFamily:lang==="en"?"Tajawal,sans-serif":"DM Sans,sans-serif",fontSize:".65rem",fontWeight:700,whiteSpace:"nowrap"}}>
                {lang==="en"?"عربي":"EN"}
              </button>
              <button onClick={()=>{setAdminPwInput("");setView("admin");}} style={{background:"none",border:"none",cursor:"pointer",color:view==="admin"?"#8B6914":"#bbb",fontSize:".9rem",padding:2}}>⚙️</button>
              <button onClick={()=>setView("cart")} style={{background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:4,color:view==="cart"?"#8B6914":"#666",position:"relative",padding:"4px"}}>
                ...
                {cartCount>0&&<span style={{position:"absolute",top:-2,right:-4,background:"#3a2006",color:"#fff",borderRadius:"50%",width:16,height:16,display:"flex",alignItems:"center",justifyContent:"center",fontSize:".55rem",fontFamily:"DM Sans,sans-serif",fontWeight:700}}>{cartCount}</span>}
              </button>
            </div>
          </div>
          {/* Row 2: Logo centered */}
          <div style={{display:"flex",alignItems:"center",justifyContent:"center",padding:"8px 12px",gap:10}}>
            ...
            <div style={{fontFamily:"Cormorant Garamond,serif",fontSize:"clamp(.9rem,2.5vw,1.3rem)",color:"#4a2e06",letterSpacing:".06em",fontWeight:600,lineHeight:1.1}}>{settings.storeName}</div>
          </div>
        </div>
      </header>
      ...
      {/* ── HOME / HERO ── */}
      {view==="home"&&(
        <div style={{background:"#1a0a02",minHeight:"100vh"}}>
          {/* Hero */}
          <div style={{position:"relative",height:"calc(100vh - 90px)",background:"linear-gradient(135deg,#2a1506 0%,#4a2e10 40%,#3a2008 70%,#1a0d03 100%)",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",width:"100%"}}>
            {/* Decorative circles */}
            ...
            <div style={{textAlign:"center",position:"relative",zIndex:2,padding:"40px 16px",animation:"up .8s ease",className:"mobile-hero"}}>
              ...
              <button className="hero-btn" onClick={()=>setView("shop")}>
                {t.heroBtn}
              </button>
            </div>
          </div>
      ...
      {/* ── SHOP ── */}
      {view==="shop"&&(
        <div style={{background:"#faf7f2",minHeight:"100vh"}} className="slide-in">

          {/* ── Hero Banner - fills the screen ── */}
          <div style={{background:"linear-gradient(150deg,#1a0a02 0%,#3a2006 50%,#1a0a02 100%)",minHeight:"45vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 20px",textAlign:"center",position:"relative",overflow:"hidden"}}>
            {/* decorative */}
            <div style={{position:"absolute",width:"70vw",height:"70vw",maxWidth:500,maxHeight:500,borderRadius:"50%",border:"1px solid rgba(212,175,106,.08)",top:"50%",left:"50%",transform:"translate(-50%,-50%)",pointerEvents:"none"}}/>
            <div style={{position:"absolute",width:"45vw",height:"45vw",maxWidth:320,maxHeight:320,borderRadius:"50%",border:"1px solid rgba(212,175,106,.12)",top:"50%",left:"50%",transform:"translate(-50%,-50%)",pointerEvents:"none"}}/>
            <div style={{fontFamily:"DM Sans,sans-serif",fontSize:".65rem",letterSpacing:".35em",color:"#c4a56a",textTransform:"uppercase",marginBottom:12}}>{isAr?"موضة محتشمة · راقية · حصرية":"MODEST · PREMIUM · EXCLUSIVE"}</div>
            <h1 style={{fontFamily:"Cormorant Garamond,serif",fontSize:"clamp(2rem,5vw,3.2rem)",color:"#d4af6a",fontStyle:"italic",fontWeight:400,marginBottom:8,letterSpacing:".04em"}}>{isAr?"تشكيلتنا":"Our Collection"}</h1>
            <div style={{width:50,height:1,background:"linear-gradient(to right,transparent,#c4a56a,transparent)",margin:"12px auto 16px"}}/>
            <p style={{fontFamily:F,color:"rgba(255,255,255,.5)",fontSize:".78rem",letterSpacing:".1em"}}>{products.length} {isAr?"منتج متاح":"Products Available"}</p>
          </div>

          {/* ── Category Tabs - sticky ── */}
          <div style={{background:"#fff",borderBottom:"1px solid #f0e8da",position:"sticky",top:90,zIndex:50,boxShadow:"0 4px 16px rgba(0,0,0,.05)"}}>
            <div style={{maxWidth:1300,margin:"0 auto",display:"flex",alignItems:"stretch",overflowX:"auto",scrollbarWidth:"none",msOverflowStyle:"none"}}>
              {ALL_SUBCATS.map(cat=>{
                const count=cat==="All"?products.length:products.filter(p=>p.subcategory===cat).length;
                const active=filterCat===cat;
                return (
                  <button key={cat} onClick={()=>setFilterCat(cat)} style={{
                    background:active?"linear-gradient(180deg,#3a2006,#5a3010)":"transparent",
                    border:"none",borderBottom:active?"none":"none",
                    color:active?"#d4af6a":"#999",
                    padding:"16px 20px 14px",
                    cursor:"pointer",whiteSpace:"nowrap",
                    fontFamily:F,fontSize:".7rem",fontWeight:active?700:500,
                    letterSpacing:".08em",transition:"all .25s",
                    display:"flex",flexDirection:"column",alignItems:"center",gap:5,
                    minWidth:80,flex:"0 0 auto",
                    boxShadow:active?"inset 0 -3px 0 #d4af6a":"none"
                  }}>
                    <span style={{fontSize:"1.2rem",filter:active?"brightness(1.2)":"grayscale(.5)"}}>{ICON[cat]||"✦"}</span>
                    <span>{cat}</span>
                    <span style={{fontFamily:"DM Sans,sans-serif",fontSize:".55rem",color:active?"rgba(212,175,106,.7)":"#ddd",fontWeight:400}}>{count}</span>
                  </button>
                );
              })}
            </div>
          ...
          {/* ── Search + Sort ── */}
          <div style={{background:"#fff",borderBottom:"1px solid #f5ede0"}}>
            <div style={{maxWidth:1300,margin:"0 auto",padding:"12px 16px",display:"flex",gap:10,alignItems:"center"}}>
              <div style={{flex:1,position:"relative"}}>
                <svg style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",color:"#c4a56a",pointerEvents:"none"}} width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={isAr?"ابحث...":"Search..."} style={{width:"100%",background:"#faf7f2",border:"1.5px solid #e8ddd0",color:"#1a1208",padding:"9px 34px 9px 14px",borderRadius:30,fontSize:".8rem",fontFamily:F,boxSizing:"border-box"}}/>
              </div>
              <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{background:"#faf7f2",border:"1.5px solid #e8ddd0",color:"#888",padding:"9px 12px",borderRadius:30,fontSize:".75rem",fontFamily:F,cursor:"pointer",flexShrink:0}}>
                <option value="default">{isAr?"ترتيب":"Sort"}</option>
          ...
          {/* ── Products Grid ── */}
          <div style={{maxWidth:1300,margin:"0 auto",padding:"20px 14px 48px",boxSizing:"border-box"}}>
            {filtered.length===0
              ?<div style={{textAlign:"center",padding:"80px 20px"}}>
                <div style={{fontSize:"3.5rem",marginBottom:14}}>🌸</div>
                <p style={{fontFamily:"Cormorant Garamond,serif",fontSize:"1.4rem",color:"#b09070",fontStyle:"italic"}}>{isAr?"لا توجد منتجات":"No products yet"}</p>
              </div>
              :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:16}}>
                {filtered.map((p,i)=><PCard key={p.id} p={p} i={i} t={t} F={F} isAr={isAr} isWished={wishlist.includes(p.id)} rating={ratings[p.id]}
                  onView={()=>{setSelProd(p);setView("product");}}
                  onAdd={()=>{const v=p.variants?.find(v=>v.stock>0);if(v)addToCart(p,v);else toast(t.outOfStock,true);}}
...
function PCard({p,i,t,F,isAr,isWished,rating,onView,onAdd,onWish}) {
  ...
  return (
    <div style={{background:"#fff",borderRadius:16,overflow:"hidden",boxShadow:"0 2px 16px rgba(58,32,6,.07)",transition:"transform .3s,box-shadow .3s",animation:`up .4s ease ${i*.04}s both`,cursor:"pointer"}}
      onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-5px)";e.currentTarget.style.boxShadow="0 12px 36px rgba(58,32,6,.14)";}}
      onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="0 2px 16px rgba(58,32,6,.07)";}}>
      {/* Image */}
      <div onClick={onView} style={{position:"relative",paddingBottom:"120%",background:"linear-gradient(160deg,#f5ede0,#ede0cc)",overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
          {img?<img src={img} alt={p.name} style={{width:"100%",height:"100%",objectFit:"cover",transition:"transform .5s"}}
            onMouseEnter={e=>e.target.style.transform="scale(1.06)"} onMouseLeave={e=>e.target.style.transform="scale(1)"}
            onError={e=>e.target.style.display="none"}/>
            :<span style={{fontSize:"3.5rem"}}>{ICON[p.subcategory]||"👗"}</span>}
        </div>
        {/* Badges */}
        {disc>0&&<div style={{position:"absolute",top:8,left:8,background:"#c0392b",color:"#fff",fontSize:".6rem",fontFamily:"DM Sans,sans-serif",fontWeight:700,padding:"3px 9px",borderRadius:20}}>-{disc}%</div>}
        {stock===0&&<div style={{position:"absolute",inset:0,background:"rgba(0,0,0,.5)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontFamily:F,fontWeight:600,fontSize:".8rem"}}>{t.outOfStock}</div>}
        {stock>0&&stock<=5&&<div style={{position:"absolute",top:8,right:36,background:"rgba(127,68,0,.9)",color:"#ffb347",fontSize:".58rem",fontFamily:"DM Sans,sans-serif",padding:"3px 8px",borderRadius:20,fontWeight:600}}>{t.onlyLeft(stock)}</div>}
        {/* Countdown */}
        {countdown&&<div style={{position:"absolute",bottom:0,left:0,right:0,background:"rgba(58,32,6,.85)",color:"#d4af6a",fontFamily:"DM Sans,sans-serif",fontSize:".6rem",padding:"5px 8px",textAlign:"center",fontWeight:600}}>
          ⏱ {countdown.d}{t.days} {countdown.h}{t.hours} {countdown.m}{t.mins} {countdown.s}{t.secs}
        </div>}
        {/* Wishlist */}
        <button onClick={e=>{e.stopPropagation();onWish();}} style={{position:"absolute",top:8,right:8,background:"rgba(255,255,255,.9)",border:"none",borderRadius:"50%",width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:".9rem",boxShadow:"0 2px 8px rgba(0,0,0,.1)",transition:"transform .2s"}}
      ...
      <div style={{padding:"12px 12px 14px"}}>
        <div style={{display:"flex",gap:4,marginBottom:5,flexWrap:"wrap"}}>
          <span style={{background:"#f5ede0",color:"#8B6914",padding:"2px 8px",borderRadius:20,fontSize:".6rem",fontFamily:"DM Sans,sans-serif",fontWeight:600}}>{p.type}</span>
        </div>
        <div onClick={onView} style={{fontFamily:"Cormorant Garamond,serif",fontSize:"1rem",fontWeight:600,marginBottom:8,lineHeight:1.3,color:"#2a1a06"}}>{p.name}</div>
        {rating?.stars&&<div style={{marginBottom:6}}><Stars rating={rating.stars} size={11}/></div>}
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:4}}>
          <div>
            <span style={{color:"#8B6914",fontFamily:"DM Sans,sans-serif",fontWeight:700,fontSize:".95rem"}}>{USD(p.price)}</span>
            {p.originalPrice&&<span style={{color:"#ddd",fontFamily:"DM Sans,sans-serif",fontSize:".75rem",textDecoration:"line-through",marginRight:4}}>{USD(p.originalPrice)}</span>}
          </div>
          <button onClick={onAdd} disabled={stock===0} style={{background:stock===0?"#eee":"linear-gradient(135deg,#3a2006,#5a3010)",color:stock===0?"#bbb":"#fff",border:"none",padding:"7px 12px",borderRadius:20,cursor:stock===0?"default":"pointer",fontFamily:F,fontSize:".68rem",fontWeight:700,transition:"all .2s",flexShrink:0,boxShadow:stock>0?"0 3px 10px rgba(58,32,6,.25)":"none"}}
            onMouseEnter={e=>stock>0&&(e.currentTarget.style.transform="scale(1.05)")} onMouseLeave={e=>e.currentTarget.style.transform=""}>
