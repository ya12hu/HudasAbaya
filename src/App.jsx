    ...
    if(!adminLoggedIn){
      return (
        <div style={{maxWidth:360,margin:"80px auto",padding:24,background:"#fff",borderRadius:12,boxShadow:"0 2px 16px rgba(0,0,0,.08)"}}>
          ...
          <input type="password" style={styles.input} placeholder={t.passwordLabel} value={adminPass} onChange={e=>setAdminPass(e.target.value)}
            onKeyDown={e=>{ if(e.key==="Enter"){ if(adminPass===settings.adminPassword){setAdminLoggedIn(true);setAdminPassErr(false);}else setAdminPassErr(true); }}} />
