...
const DEFAULT_PRODUCTS = [
  ...HM_IMGS.map((img,i)=>({
    id:`hm${i+1}`, category:"hijabMagnets", name:"Hijab Magnets", nameAr:"مغناطيسات الحجاب",
    image:img, price:0, discount:0, active:true, newArrival:true, stock:99,
  })),
  ...PM_IMGS.map((img,i)=>({
    id:`pm${i+1}`, category:"printedModal", name:"Printed Modal Hijab", nameAr:"حجاب مودال مطبوع",
    image:img, price:0, discount:0, active:true, newArrival:false, stock:99,
  })),
];
...
const styles = {
  app:{ fontFamily:"'Cormorant Garamond',serif", background:"#faf9f7", minHeight:"100vh" },
  ...
  wishBtn:{ position:"absolute", background:"rgba(255,255,255,.9)", border:"none", borderRadius:50, width:30, height:30, cursor:"pointer", fontSize:".9rem", display:"flex", alignItems:"center", justifyContent:"center" },
  detailWrap:{ maxWidth:900, margin:"0 auto", padding:20, display:"grid", gap:30 },
...
function ProductCard({p, t, lang, getFinalPrice, wishlist, toggleWishlist, addedMap, addToCart, isRTL, onOpen}){
  const finalPrice = getFinalPrice(p);
  const hasDisc = finalPrice < p.price;
  ...
  return (
    <div style={{...styles.card, position:"relative"}}
      ...
      <div style={{position:"relative"}} onClick={()=>onOpen(p)}>
        <img src={p.image} alt={name} style={styles.cardImg} loading="lazy" />
        {p.newArrival && <span style={styles.badge}>{t.newArrival}</span>}
        {hasDisc && p.price>0 && <span style={styles.saleBadge}>{t.sale}</span>}
      </div>
      <button style={{...styles.wishBtn, [isRTL?"left":"right"]:8, top:hasDisc&&p.price>0?36:8}}
...
function ShopPage({t, search, setSearch, categoryFilter, setCategoryFilter, visibleProducts, cardProps}){
  return (
    <>
      <div style={styles.hero}>
        <div style={styles.heroTitle}>{cardProps.storeName}</div>
        <div style={styles.heroSub}>{cardProps.storeTagline}</div>
...
function ProductPage({p, t, products, cardProps, onBack}){
  if(!p) return null;
  const finalPrice = cardProps.getFinalPrice(p);
  const hasDisc = finalPrice < p.price;
  const name = cardProps.lang==="ar"?p.nameAr:p.name;
  const related = products.filter(x=>x.active&&x.category===p.category&&x.id!==p.id).slice(0,4);
  const narrow = typeof window!=="undefined" && window.innerWidth<640;
  return (
    <div>
      <button style={{...styles.navBtn, padding:"16px 20px", color:"#1a1a1a"}} onClick={onBack}>{t.back}</button>
      <div style={{...styles.detailWrap, gridTemplateColumns: narrow?"1fr":"1fr 1fr"}}>
        <img src={p.image} alt={name} style={styles.detailImg} />
        <div>
          <div style={styles.detailName}>{name}</div>
          ...
          <button style={{...styles.detailAddBtn,...(cardProps.addedMap[p.id]?{background:"#2d7a2d"}:{})}}
            onClick={()=>cardProps.addToCart(p)}>
            {cardProps.addedMap[p.id]?t.addedToCart:t.addToCart}
          </button>
          <button style={{...styles.detailAddBtn,background:"none",color:"#1a1a1a",border:"1px solid #ddd",marginTop:8}}
            onClick={()=>cardProps.toggleWishlist(p.id)}>
            {cardProps.wishlist.includes(p.id)?t.removeWishlist:t.addWishlist}
          </button>
        </div>
      </div>
      {related.length>0 && (
        <div style={{padding:"20px 20px 40px"}}>
          ...
          <div style={styles.grid}>{related.map(rp=><ProductCard key={rp.id} p={rp} {...cardProps}/>)}</div>
        </div>
      )}
    </div>
  );
...
function CartPage({t, cart, products, getFinalPrice, updateQty, removeFromCart, settings, couponInput, setCouponInput, cartTotal, tax, shippingCost, orderTotal, onCheckout, onShop, lang}){
  return (
    <div style={styles.cartWrap}>
      ...
      {cart.length===0
        ? <div style={{textAlign:"center",padding:60,color:"#999"}}>
            ...
            <div>{t.emptyBag}</div>
            <button style={{...styles.shopBtn,marginTop:20}} onClick={onShop}>{t.continueShopping}</button>
          </div>
        : <>
          {cart.map(item=>{
            ...
            return (
              <div key={item.id} style={styles.cartItem}>
                <img src={p.image} alt={name} style={styles.cartImg} />
                <div style={{flex:1}}>
          ...
          <div style={{background:"#fff",borderRadius:12,padding:16,boxShadow:"0 1px 8px rgba(0,0,0,.06)"}}>
            ...
            <button style={{...styles.checkoutBtn,marginTop:16}} onClick={onCheckout}>{t.checkout}</button>
          </div>
        </>
      }
    </div>
...
function CheckoutPage({t, shippingForm, setShippingForm, formError, setFormError, orderTotal, settings, onBack, onPlaceOrder}){
  ...
  return (
    <div style={styles.checkoutWrap}>
      <button style={{...styles.navBtn,padding:"16px 0",color:"#1a1a1a"}} onClick={onBack}>{t.back}</button>
      ...
      {settings.paypalClientId ? (
        ...
        <div>
          ...
          <button style={styles.checkoutBtn} onClick={()=>{ if(!validate()){setFormError(t.fillFields);return;} setFormError(""); onPlaceOrder(); }}>
            {t.checkout}
          </button>
        </div>
      )}
...
function ConfirmPage({t, order, onShop}){
  return (
    <div style={{maxWidth:500,margin:"60px auto",padding:20,textAlign:"center"}}>
      ...
      {order && <p style={{color:"#999",fontSize:".85rem"}}>Order ID: {order.id}</p>}
      <button style={{...styles.shopBtn,marginTop:24}} onClick={onShop}>{t.continueShopping}</button>
    </div>
  );
}

function AdminLogin({t, adminPass, setAdminPass, adminPassErr, onLogin}){
  return (
    <div style={{maxWidth:360,margin:"80px auto",padding:24,background:"#fff",borderRadius:12,boxShadow:"0 2px 16px rgba(0,0,0,.08)"}}>
      ...
      <input type="password" autoFocus style={styles.input} placeholder={t.passwordLabel} value={adminPass}
        onChange={e=>setAdminPass(e.target.value)}
        onKeyDown={e=>{ if(e.key==="Enter") onLogin(); }} />
      {adminPassErr && <div style={{color:"#e53935",fontSize:".85rem",marginBottom:8}}>{t.wrongPass}</div>}
      <button style={styles.checkoutBtn} onClick={onLogin}>{t.login}</button>
    </div>
  );
}

function AdminPanel({t, products, updateProduct, settings, updateSetting, updateCatDisc, orders, setOrders, savedMsg, saveAdmin, adminTab, setAdminTab, exportCSV, onLogout}){
  return (
    <div style={styles.adminWrap}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <h2 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"1.8rem"}}>{t.adminPanel}</h2>
        <button style={{...styles.navBtn,color:"#e53935"}} onClick={onLogout}>{t.logout}</button>
      </div>
...
export default function App() {
  ...
  const [displayPage, setDisplayPage] = useState("shop"); // for cross-fade
  ...
  // Cinematic cross-fade page transition
  ...
  const getFinalPrice = (p, qty=1) => calcPrice(p, settings, qty, couponInput);

  const cartTotal = cart.reduce((s,i)=>s+getFinalPrice(products.find(p=>p.id===i.id)||{price:0,discount:0,category:""},i.qty)*i.qty,0);
  ...
  function saveAdmin(key){ setSavedMsg(m=>({...m,[key]:true})); setTimeout(()=>setSavedMsg(m=>({...m,[key]:false})),2000); }
  function updateProduct(id,field,val){ setProducts(ps=>ps.map(p=>p.id===id?{...p,[field]:val}:p)); }
  ...
  function tryAdminLogin(){
    if(adminPass===settings.adminPassword){ setAdminLoggedIn(true); setAdminPassErr(false); }
  ...
  function placeOrder(){
    const order = {
      ...
      items:[...cart], shipping:shippingForm, total:orderTotal.toFixed(2), status:"pending",
    };
    setOrders(o=>[...o,order]);
  ...
  const cardProps = {
    t, lang, isRTL, getFinalPrice, wishlist, toggleWishlist, addedMap, addToCart,
    onOpen:(p)=>{ setSelectedProduct(p); navigate("product"); },
    storeName:settings.storeName, storeTagline:settings.storeTagline,
  };
  ...
  return (
    <div style={{...styles.app, direction:isRTL?"rtl":"ltr"}}>
      <style>{`
        ...
        .page-transition { animation: fadeInUp .45s cubic-bezier(.22,1,.36,1); }
        .page-fading-out { opacity:0; transition:opacity .25s ease; }
        .page-fading-in { opacity:1; transition:opacity .35s ease; }
      `}</style>
      {/* Header */}
        ...
        <div style={styles.headerBottom}>
          {[["shop",t.store],["cart",t.shoppingBag],["admin",t.admin]].map(([p2,label])=>(
            <button key={p2} style={{...styles.navBtn,...(page===p2||( p2==="shop"&&["shop","product"].includes(page))?styles.navBtnActive:{})}}
              onClick={()=>navigate(p2)}>{label}</button>
          ))}
          {wishlist.length>0 && (
            <button style={{...styles.navBtn}} onClick={()=>{ setCategoryFilter("all"); setSearch(""); navigate("shop"); }}>
      ...
      {/* Pages with cinematic cross-fade */}
      <div className={fading ? "page-fading-out" : "page-fading-in page-transition"}>
        {displayPage==="shop" && <ShopPage t={t} search={search} setSearch={setSearch} categoryFilter={categoryFilter} setCategoryFilter={setCategoryFilter} visibleProducts={visibleProducts} cardProps={cardProps} />}
        {displayPage==="product" && <ProductPage p={selectedProduct} t={t} products={products} cardProps={cardProps} onBack={()=>navigate("shop")} />}
        {displayPage==="cart" && <CartPage t={t} cart={cart} products={products} getFinalPrice={getFinalPrice} updateQty={updateQty} removeFromCart={removeFromCart} settings={settings} couponInput={couponInput} setCouponInput={setCouponInput} cartTotal={cartTotal} tax={tax} shippingCost={shippingCost} orderTotal={orderTotal} onCheckout={()=>navigate("checkout")} onShop={()=>navigate("shop")} lang={lang} />}
        {displayPage==="checkout" && <CheckoutPage t={t} shippingForm={shippingForm} setShippingForm={setShippingForm} formError={formError} setFormError={setFormError} orderTotal={orderTotal} settings={settings} onBack={()=>navigate("cart")} onPlaceOrder={placeOrder} />}
        {displayPage==="confirm" && <ConfirmPage t={t} order={confirmOrder} onShop={()=>navigate("shop")} />}
        {displayPage==="admin" && (
          adminLoggedIn
            ? <AdminPanel t={t} products={products} updateProduct={updateProduct} settings={settings} updateSetting={updateSetting} updateCatDisc={updateCatDisc} orders={orders} setOrders={setOrders} savedMsg={savedMsg} saveAdmin={saveAdmin} adminTab={adminTab} setAdminTab={setAdminTab} exportCSV={exportCSV} onLogout={()=>setAdminLoggedIn(false)} />
            : <AdminLogin t={t} adminPass={adminPass} setAdminPass={setAdminPass} adminPassErr={adminPassErr} onLogin={tryAdminLogin} />
        )}
      </div>
    </div>
  );
